...
def main(args):
    ...
    try:
        ...
        new_tables =[]
        print("No new table schema requested for this run")
    ...
    if len(updated_resource) > 0:
        for resourcename in updated_resource:
            ...
            # This is alternative method being use previously, using Glue crawler derived schema
            ...
            ## schema_from_file should be available since the same schema was created when ingesting ndjson
            athena_delta_from_schema( athena_tb = resourcename
                                     #, schema = get_crawled_schema(resourcename)
                                     , schema = schema_from_file(resourcename)
                                     , s3_data_input = s3lakepath
                                     , logpath= s3lakepath + 'athena_logs'
                                     , athena_db = "delta")

        #print("Table schema is updated in {}".format(new_tables))
        print("Table {} is updated ".format(resourcename))  
